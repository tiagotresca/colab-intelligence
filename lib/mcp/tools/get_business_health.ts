// MCP tool: get_business_health
//
// Estado consolidado de uma empresa nos últimos 30 dias. Headline + KPIs
// chave + sinais detectados (anomalias, oportunidades). Pensado para ser
// a primeira coisa que o agente chama quando vai propor tarefas.
//
// Output decision-ready (<2k tokens em qualquer caso real).
//
// Nota sobre localização: CLAUDE.md sugere `api/mcp/tools/`, mas no Vercel
// qualquer ficheiro em `api/` que não exporte default falha o build. Por
// isso vivemos em `lib/mcp/tools/`. O HTTP entrypoint continua em `api/mcp.ts`.

import { z } from 'zod';
import { supabase } from '../../supabase.js';
import type { BusinessHealthOutput, Channel } from '../../types.js';
